...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (isModeratorLogin) {
        // Moderator login uses a separate endpoint
        const response = await axios.post(
          `${serverURL.url}moderator/login`,
          { email, password },
        ...
        if (!data.success) {
          throw new Error(data.message || "Moderator login failed");
        }

        // Store moderator data
        const userData = data.moderator || data.user || data.data || {};
        userData.role = "moderator";
        if (data.eventId) userData.eventId = data.eventId;
        ...
        // Reload to pick up the new user in AuthProvider
        ...
        <div className="bg-white/20 backdrop-blur-xl border border-white/30 shadow-2xl rounded-3xl p-8 w-full max-w-md mx-auto">
          <div className="flex flex-col items-center justify-center text-center mb-6">
            ...
            <p className="text-sm text-orange-50 text-center">
              {isModeratorLogin ? "Sign in as event moderator" : "Sign in to access your tickets and upcoming events"}
            </p>
          </div>

          {/* Login Type Toggle */}
          {/* <div className="flex rounded-lg overflow-hidden mb-4 border border-white/30">
            <button
              type="button"
              onClick={() => { setIsModeratorLogin(false); setError(""); }}
            ...
            <button
              type="button"
              onClick={() => { setIsModeratorLogin(true); setError(""); }}
              className={`flex-1 py-2 text-sm font-medium transition-all duration-200 ${
                isModeratorLogin
                  ? "bg-orange-500 text-white"
                  : "bg-white/10 text-white/70 hover:bg-white/20"
            ...
            <div className="form-control">
              ...
              <div className="relative">
                ...
                <button
                  ...
                  className="absolute inset-y-0 right-0 flex items-center pr-3 text-gray-500 hover:text-orange-500 transition-colors duration-200"
                >
              ...
              <div className="form-control">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="pl-9 pr-9 w-full py-2.5 px-3 rounded-lg bg-white/90 border border-orange-200 shadow-sm focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent transition duration-300"
                    minLength="6"
                    required
                  />
                  <button
